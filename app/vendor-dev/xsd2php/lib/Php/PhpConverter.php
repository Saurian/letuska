...
//            $this->visitType($type);
        ...
        foreach ($schema->getElements() as $index => $element) {

//            if (!Strings::endsWith($index, 'Req')) {
//                continue;
//            }

            if (
//                $element->getName() != 'AirCreateReservationReq' and
//                $element->getName() != 'AirCreateReservationRsp'
                $element->getName() != 'AirPriceReq' and
                $element->getName() != 'AirPriceRsp'
//                $element->getName() != 'LowFareSearchReq' and
//                $element->getName() != 'LowFareSearchRsp'
//                $element->getName() != 'AvailabilitySearchReq' and
//                $element->getName() != 'AvailabilitySearchRsp'
            ) { //LowFareSearchReq  AvailabilitySearchReq  PingReq  AirCreateReservationReq
                continue;
            }

            $class = $this->visitElementDef($element);

            if (Strings::endsWith($index, 'Req')) {
                $class->setDependencies(array_values(array_unique($this->priorities)));
            }


//            die(dump($this->classes));

//            die(dump(array_values(array_unique($this->priorities))));
            $this->priorities = [];
...
//            break;
    ...
    private function visitGroup(PHPClass $class, Schema $schema, Group $group)
    {
        foreach ($group->getElements() as $childGroup) {
            ...
            $fullName = $this->namespaces[$schema->getTargetNamespace()] . '\\' . $childGroup->getName();
            $this->priorities[] = $fullName;
    ...
    private function visitAttributeGroup(PHPClass $class, Schema $schema, AttributeGroup $att)
    {
        foreach ($att->getAttributes() as $childAttr) {
            if ($childAttr instanceof AttributeGroup) {
                ...
                $property->getType()->setXsdNamespace($this->namespaces[$schema->getTargetNamespace()]);
                $class->addProperty($property);
            }
        }
    }
...
//            if ($element->getName() == 'SearchAirLeg') {
...
//            die();
            ...
            $this->priorities[] = $class->getFullName();
            ...
            $class->setXsdNamespace($this->namespaces[$element->getSchema()->getTargetNamespace()]);
        }
    ...
    private function visitType(Type $type, $force = false)
    {
        if (! isset($this->classes[spl_object_hash($type)])) {
            ...
            if ($alias = $this->getTypeAlias($type)) {
                $class->setName($alias);
                $class->setXsdNamespace($this->namespaces[$type->getSchema()->getTargetNamespace()]);
...
//            dump($element->getName());
            ...
            $fullName= $this->namespaces[$element->getSchema()->getTargetNamespace()] . '\\' . $element->getName();
//            dump($fullName);
    ...
    private function handleClassExtension(PHPClass $class, Type $type)
    {
        if ($alias = $this->getTypeAlias($type)) {
            ...
            $val->setType($c);
            $val->getType()->setXsdNamespace($this->namespaces[$type->getSchema()->getTargetNamespace()]);
            $c->addProperty($val);
            $class->setExtends($c);
        } else {
            $extension = $this->visitType($type, true);
            $extension->setXsdNamespace($this->namespaces[$type->getSchema()->getTargetNamespace()]);
    ...
    {
        $property = new PHPAttribute();
        $property->setName(Inflector::camelize($attribute->getName()));
        $property->setUse($attribute->getUse());
...
//        $element->getType()->
//        dump($this->namespaces[$element->getSchema()->getTargetNamespace()]);
...
//                dump($t->getName());
                ...
                $property->getType()->setName($classType->getName());
                $property->getType()->setNamespace($classType->getNamespace());
                $property->getType()->setXsdNamespace($this->namespaces[$element->getSchema()->getTargetNamespace()]);
...
//                dump($class->getName());
                ...
                if ($element->getName() == 'SearchOrigin') {
...
//            (dump($node->getName()));
//            dump($node->getReferencedElement()->getType()->getName());
//            dump($class);
//            die();
...
//            (dump($node->getName()));
//            dump($class->getFullName());

//            dump($node->getName());

//            dump($this->namespaces[$node->getSchema()->getTargetNamespace()]);

//            (dump($class->getFullName()));
            if ($node->getName() == 'SearchOrigin') {
